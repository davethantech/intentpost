...
app.use(cors({ origin: allowedOrigins.length ? allowedOrigins : false, credentials: true, methods: ["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"] }));
app.use(express.json({ limit: "1mb", strict: true })); app.use(cookie()); app.use(general);
app.get("/api/health", (req, res) => res.json({ ok: true, service: "intentpost-api", version: "3.0.0" }));
app.get("/api/ready", async (req, res) => { try { await db.query("SELECT 1"); res.json({ ready: true }); } catch { res.status(503).json({ ready: false }); } });
app.use("/api/auth", require("./routes/auth")); app.use("/api/webhooks", require("./routes/webhooks")); app.use("/api/graph", require("./routes/graph")); app.use("/api/ai", require("./routes/ai")); app.use("/api/campaigns", require("./routes/campaigns")); app.use("/api/fulfillment", require("./routes/fulfillment")); app.use("/", require("./routes/attribution"));
...
if (require.main === module) {
  ...
  const shutdown = async signal => { console.log(JSON.stringify({ event: "shutdown", signal })); await db.close(); process.exit(0); };
